...
from dashboard.windows import StationMessage, SEFDs
...
class Dashboard(QMainWindow):
    ...
    def __init__(self, ses_id):
        ...
        self.timer, self.messages = None, None
    ...
    def make_session_box(self):
        ...
        self.session_box = box = QGridLayout()
        ...
        version = f'V{schedule.version} available {schedule.updated.strftime("%Y-%m-%d %H:%M")}' if schedule else 'None'
        self.version_box = make_text_box(version, fit=False)
        box.addWidget(self.version_box, 2, 1, 1, 2)

        skdButton = QPushButton('Make SKD')
        #skdButton.clicked.connect(self.show_scheduler)
        box.addWidget(skdButton, 2, 5)
    ...
    def update_session_box(self):
        ...
        version = f'V{self.session.schedule.version} available {self.session.schedule.updated.strftime("%Y-%m-%d %H:%M")}' if self.session.schedule else 'None'
        self.version_box.setText(version)
    ...
    def process_sta_info(self, headers, data):
        ...
        ses_id = data.get('session', headers.get('session', '__')).upper()
    ...
    def msg_received(self, ch, method, properties, body):
        print(body.decode().strip())  # close connection on receiving stop message
        ch.basic_ack(delivery_tag=method.delivery_tag)

    def update_session(self):
        self.get = Get('DB', f'/sessions/{self.session.code}')
        self.get.on_finish(self.session.code, self.process_session_response)
        self.get.start()

    def get_schedule(self, next=None):
        self._after_get_schedule = next
        self.get = Get('DB', f'/schedules/{self.session.code.lower()}', params={'select': 'summary'})
        self.get.on_finish(self.session.code, self.process_schedule_response)
        self.get.start()
    ...
    def get_sefds(self, station=None):
        ...
        for sta_id in [station] if station else self.network:
            ...
            if not self.sefds[sta_id]['data']:
                action = (sta_id, f'/data/onoff/{sta_id}', params)
                requests.append(action)

        # Use MultiGet to request information from VWS
        if requests:
            self._sefds = MultiGet('DB')
            self._sefds.set_requests(requests, self.process_sefds, self.error_sefds, self.update_monit_box)
            self._sefds.start()
